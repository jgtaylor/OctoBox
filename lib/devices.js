...
module.exports = (config) => {
	...
	devices.query = (search) => {
		...
		return Object.keys(devices.dev).filter(
			(el) => {
				let found;
				...
				Object.keys(D).forEach(
					(i) => {
						...
						// } else if ( DI === search ) {
							...
							console.log("String detected, found: %s, based on the search pattern %s ", finding, searchRe);
							if (finding) return found = el;
						} else if ( typeof DI === "object") {
							if (Object.values(DI).includes(search)) return found = el;
						}
					});
				if (found) return el;
				// }
	...
	devices.on("added", (msg) => {
		console.log("added: %s", msg.deviceID);
	});
